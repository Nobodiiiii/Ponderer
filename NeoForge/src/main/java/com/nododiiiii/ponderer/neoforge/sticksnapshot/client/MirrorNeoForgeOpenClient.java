...
public class MirrorNeoForgeOpenClient {
    ...
    private MirrorNeoForgeOpenClient() {
    }

    public static void open(MirrorNeoForgeOpenPacket msg) {
        try {
            ...
            if (menuType == null) {
                StickSnapshotFeature.LOGGER.warn("neoforge-open failed: unknown menuTypeId={}", msg.menuTypeId());
                return;
            }
            ...
            RegistryFriendlyByteBuf extraData =
                    new RegistryFriendlyByteBuf(Unpooled.wrappedBuffer(msg.extraData()), mc.player.registryAccess());
            MenuScreens.getScreenFactory(menuType).ifPresentOrElse(screenFactory -> {
                ...
                if (menu == null) {
                    StickSnapshotFeature.LOGGER.warn(
                            "neoforge-open failed: menu factory returned null windowId={} key={}",
                ...
                if (mc.screen instanceof PonderUI) {
                    ...
                    "neoforge-open failed: no screen factory windowId={} key={}",
                    msg.windowId(), menuKey));
        } catch (Exception ex) {
            Minecraft mc = Minecraft.getInstance();
            ...
            StickSnapshotFeature.LOGGER.warn(
                    "neoforge-open client exception for menuTypeId={} requestedPos={} bePresent={} state={}",
        ...
        try {
            ...
            if (injectedClientBlock.originalBlockEntityTag != null) {
                BlockEntity restored = BlockEntity.loadStatic(injectedClientBlock.pos, injectedClientBlock.originalState,
                        injectedClientBlock.originalBlockEntityTag, mc.level.registryAccess());
                if (restored != null) {
    ...
    private static void ensureClientTileContext(MirrorNeoForgeOpenPacket msg) {
        Minecraft mc = Minecraft.getInstance();
        ...
        CompoundTag originalBeTag =
                originalBe != null ? originalBe.saveWithFullMetadata(mc.level.registryAccess()) : null;
        ...
        if (msg.snapshotBlockEntityTag() != null) {
            ...
            BlockEntity replayBe = BlockEntity.loadStatic(pos, snapshotState, beTag, mc.level.registryAccess());
            if (replayBe != null) {
